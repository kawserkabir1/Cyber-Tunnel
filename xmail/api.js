...
function warningAlert() {
  alert(
    "⚠️ Don't Refresh page. This project is purely for educational purposes. We do not allow illegal things to be done with this project and we are not responsible for any incidents that may occur. This project use 1secmail's api for create e-mails. Use it legally ⚠️"
  );
}
      ...
      for (const email of emails) {
        emailsElement.append(`
            <tr>
              ...
              <td>${email.subject}</td>
              <td>${email.date}</td>
              <td id="${email.id}"><a onclick="loadEmail('${email.id}')" style="color: #fff;">Load content...</a></td>
            </tr>
          `);
      }
    }
